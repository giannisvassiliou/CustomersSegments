...
class FixedOnlineKMeans:
    def __init__(self, max_clusters=7, min_cluster_size=15, distance_threshold=0.6, learning_rate=0.2):
...
class FixedOnlineClusteringEngine:
    ...
    def __init__(self, max_clusters=7, min_cluster_size=25):
        self.max_clusters = max_clusters
        self.min_cluster_size = min_cluster_size
        ...
        self._lock = threading.RLock()        
        self.last_centroids = None  # warm-start storage for batch KMeans


    
    def _behavior_metrics_from_centroid(self, centroid: np.ndarray, size: int) -> Dict[str, float]:
        # \"\"\"Compute basic business metrics from an 8-dim centroid (0..1-ish).\"\"\"
        # # Defensive guards
        if centroid is None or len(centroid) < 8:
            return {
                'avg_session_duration': 300.0,
                'avg_pages_per_session': 5.0,
                'conversion_rate': 0.05,
                'avg_revenue_per_user': 30.0,
                'total_revenue': 30.0 * size,
                'avg_sessions_per_user': 3.0,
                'primary_device': 'desktop',
                'engagement_score': 0.5,
                'loyalty_score': 0.5
            }
        avg_session_duration = float(centroid[0]) * 1800.0
        avg_pages_per_session = float(centroid[1]) * 20.0
        conversion_rate = float(centroid[2])
        avg_revenue_per_user = float(centroid[3]) * 200.0
        total_revenue = avg_revenue_per_user * size
        return {
            'avg_session_duration': avg_session_duration,
            'avg_pages_per_session': max(2.0, avg_pages_per_session),
            'conversion_rate': conversion_rate,
            'avg_revenue_per_user': avg_revenue_per_user,
            'total_revenue': total_revenue,
            'avg_sessions_per_user': max(1.0, float(centroid[6]) * 5.0),
            'primary_device': 'mobile' if centroid[4] >= 0.5 else 'desktop',
            'engagement_score': float(centroid[5]),
            'loyalty_score': float(centroid[7]),
        }

    
    def batch_kmeans_segments(self, user_profiles: Dict[str, Any]) -> List['SegmentData']:
        # """Run standard batch KMeans on ALL users, warm-starting from previous centroids if available.
        #    Expects user_profiles to be a thread-safe snapshot (dict copy)."""
        try:
            # Build matrix X and user id list
            X = []
            user_ids = []
            for uid, profile in user_profiles.items():
                vec = self._get_optimized_behavior_vector(profile) if hasattr(self, '_get_optimized_behavior_vector') else None
            ...
            N, d = X.shape
            k = int(max(2, min(self.max_clusters, int(np.sqrt(N)))))

            # Warm-start logic
            use_warm = (
                self.last_centroids is not None
                and isinstance(self.last_centroids, np.ndarray)
                and self.last_centroids.shape == (k, d)
                and np.all(np.isfinite(self.last_centroids))
            )
            if use_warm:
                km = KMeans(
                    n_clusters=k,
                    init=self.last_centroids,
                    n_init=1,            # when passing explicit centers
                    max_iter=100,
                    random_state=42,
                    algorithm="lloyd",
                )
            ...
            self.last_centroids = km.cluster_centers_.astype(np.float32, copy=True)

            # Build segments
            segments_map: Dict[int, List[int]] = {}
            for i, lab in enumerate(labels):
                segments_map.setdefault(int(lab), []).append(i)

            new_segments: List['SegmentData'] = []
            now = datetime.now()
            for lab, idxs in segments_map.items():
                size = len(idxs)
                if size < self.min_cluster_size:
                    continue

                centroid = X[idxs].mean(axis=0)
                behavioral_metrics = self._behavior_metrics_from_centroid(centroid, size)
                seg_id = f"kmeans_{lab}"

                segment = SegmentData(
                    segment_id=seg_id,
                    segment_name=f"Batch Segment {lab}",
                    user_ids=set(user_ids[i] for i in idxs),
                    centroid=centroid,
                    timestamp=now,
                    size=size,
            ...
            # Optional: per-segment silhouette
            try:
                if len(new_segments) >= 2:
                    scores = self._calculate_silhouette_scores(new_segments)
                    for seg in new_segments:
                        seg.silhouette_score = scores.get(seg.segment_id, 0.0)
            except Exception:
                pass

            return new_segments
        except Exception as e:
            logging.getLogger(__name__).error(f"batch_kmeans_segments failed: {e}")
            return []
...
# Also add this alternative method for when you have actual user feature data
    ...
    def process_user_profile(self, profile: UserProfile) -> Tuple[int, List[SegmentData]]:
        ...
        with self._lock:
            ...
            # Online KMeans disabled in batch mode; segments are computed in monitoring loop
            current_segments = []
...
class FixedOnlineMonitoringSystem:
    ...
    def __init__(self, user_generation_rate=20.0, monitoring_interval=5):
    ...
    def _online_processing_loop(self):
        """Profiles-only processing loop (no online KMeans)."""
        logger.info("Starting processing loop (profiles-only, no online KMeans)")
        while self.is_running:
            try:
                processed_count = 0
                while not self.activity_queue.empty() and processed_count < 130:
                    ...
                    try:
                        features = self.clustering_engine._get_optimized_behavior_vector(profile)
                        if features is not None and np.all(np.isfinite(features)):
                            with self.clustering_engine._lock:
                                self.clustering_engine.feature_buffer.append(features)
                    except Exception:
                        pass
                    processed_count += 1
                time.sleep(0.05)
            except Exception as e:
                logger.error(f"Processing loop error (profiles-only): {e}")
    ...
    def _monitoring_loop(self):
        ...
        while self.is_running:
            try:
                # Recompute clusters in *batch* using all user data
                with self.profile_manager._lock:
                    profiles_snapshot = dict(self.profile_manager.user_profiles)

                    logger.info("Running batch KMeans on %d users", len(profiles_snapshot))
                    new_segments = self.clustering_engine.batch_kmeans_segments(profiles_snapshot)
                with self._lock:
...
class FixedDemo:
    ...
    def _print_status_with_notifications(self):
        ...
        print(f"\nUSER ACCOUNTING:")
        print(f"  Total unique Users: {user_acc['unique_users_total']}")
